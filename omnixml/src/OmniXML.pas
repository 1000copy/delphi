...
type
  ...
  IXMLNode = interface
    ...
    procedure SelectSingleNode(Pattern: XmlString; var Result: IXMLNode); overload;
    function SelectSingleNode(Pattern: XmlString): IXMLNode; overload;
    function SelectSingleElement(Pattern: XmlString): IXMLElement;//ONDREJ
    function SelectSingleElementNil(Pattern: XmlString): IXMLElement;//ONDREJ
    function SelectSingleElementCreate(aTagName: XmlString): IXMLElement;//ONDREJ
    procedure SelectNodes(Pattern: XmlString; var Result: IXMLNodeList); overload;
    function SelectNodes(Pattern: XmlString): IXMLNodeList; overload;
    ...
    procedure SetAttribute(const Name, Value: XmlString);//ondrej
    procedure RemoveAttribute(const Name: XmlString);//ondrej
...
//    procedure SetDocType(const Value: IXMLDocumentType);
    ...
    function LoadFromStream(const Stream: TStream; const ForceXMLEncoding: Word = 0): Boolean;
...
type
  ...
  protected
    ...
    procedure SelectNodes(Pattern: XmlString; var Result: IXMLNodeList); overload; virtual;
    procedure SelectSingleNode(Pattern: XmlString; var Result: IXMLNode); overload; virtual;
  public
    ...
    procedure WriteToStream(const OutputStream: IUnicodeStream);
    function SelectNodes(Pattern: XmlString): IXMLNodeList; overload; virtual;
    function SelectSingleNode(Pattern: XmlString): IXMLNode; overload; virtual;
    function SelectSingleElement(Pattern: XmlString): IXMLElement;//ONDREJ
    function SelectSingleElementNil(Pattern: XmlString): IXMLElement;//ONDREJ
    function SelectSingleElementCreate(aTagName: XmlString): IXMLElement;//ONDREJ
...
*)
  ...
  public
    ...
    constructor Create(aParent: IXMLNode);
    //property Values[const Name: XmlString]: XmlString read GetValue write SetValue; default;//ondrej
  end;
...
function CreateXMLDoc(const aRootNodeName: String): IXMLDocument;
begin
  Result := CreateXMLDoc;
...
begin
  ...
  begin
    {$IFDEF FPC}
    Result := FXTS.WrappedStream.Read(ReadChar, 1) > 0;
    {$ELSE}
...
constructor TXMLAttrList.Create(aParent: IXMLNode);
begin
  inherited Create;
...
begin
  ...
  if not Assigned(xA) then begin
    xA := Parent.OwnerDocument.CreateAttribute(Name);
    Add(xA);
  end;
  xA.Value := Value
end;
...
begin
  ...
  try
    {$IFDEF FPC}
    US := TXMLTextStream.Create(Stream, smWrite, CP_UTF8);
    {$ELSE}
    ...
    try
      ...
      Stream.Read(PXmlChar(Result)^, Stream.Size);
      {$ELSE}
...
procedure TXMLNode.SelectNodes(Pattern: XmlString; var Result: IXMLNodeList);
begin
  Result := XPathSelect(Self, Pattern);
end;

function TXMLNode.SelectNodes(Pattern: XmlString): IXMLNodeList;
begin
  SelectNodes(Pattern, Result);
end;

procedure TXMLNode.SelectSingleNode(Pattern: XmlString; var Result: IXMLNode);
var
...
function TXMLNode.SelectSingleElement(Pattern: XmlString): IXMLElement;
var xRes: IXMLNode;
begin
  SelectSingleNode(Pattern, xRes);
...
function TXMLNode.SelectSingleElementCreate(aTagName: XmlString): IXMLElement;
var xRes: IXMLNode;
begin
  SelectSingleNode(aTagName, xRes);
  if Assigned(xRes) and Supports(xRes, IXMLElement) then
    Result := xRes as IXMLElement
  else
...
function TXMLNode.SelectSingleElementNil(Pattern: XmlString): IXMLElement;
var xRes: IXMLNode;
begin
  SelectSingleNode(Pattern, xRes);
  if Assigned(xRes) and Supports(xRes, IXMLElement) then
...
function TXMLNode.SelectSingleNode(Pattern: XmlString): IXMLNode;
begin
...
begin
  ...
  if (FOwnerDocument.WhiteSpaceHandling = wsAutoTag) and (Value <> '') and
    (CharIs_WhiteSpace(Value[1]) or (CharIs_WhiteSpace(Value[Length(Value)])))
  then
...
procedure TXMLNode.RemoveAttribute(const Name: XmlString);
var
  i: Integer;
begin
  if not HasAttributes then
...
procedure TXMLNode.SetAttribute(const Name, Value: XmlString);
var
  Attr: IXMLAttr;
begin
      ...
      psEndData:
        begin
          ...
          begin
            ...
            if (FOwnerDocument.FTextStreamForceEncoding = 0) and FindEncoding(Self, CodePage) then//ONLY IF WE DON'T FORCE ANY ENCODING
...
begin
  Stream := TMemoryStream.Create;
  try
    //Stream.Write(HEADER_UTF16, SizeOf(HEADER_UTF16));//ONDREJ
    Stream.Write(PXmlChar(XML)^, Length(XML) * SizeOf(XmlChar));
    Stream.Seek(0, soFromBeginning);
    Result := LoadFromStream(Stream, CP_UTF16);
  finally
    Stream.Free;
  end;
end;
...
begin
  ...
  try
    ...
    Result := LoadFromStream(MS, 0);
  finally
    MS.Free;
  end;
end;

function TXMLDocument.LoadFromStream(const Stream: TStream; const ForceXMLEncoding: Word = 0): Boolean;
var
  XTS: TXMLTextStream;
...
begin
  ...
  FTextStreamForceEncoding := ForceXMLEncoding;
  if ForceXMLEncoding > 0 then
    XTS := TXMLTextStream.Create(Stream, smRead, ForceXMLEncoding)
  else
