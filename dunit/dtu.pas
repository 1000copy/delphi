...
function RunTests(exitBehavior: TRunnerExitBehavior = rxbContinue): TTestResult; overload;
implementation
uses
{$IFDEF LINUX}
  Libc,
...
begin
  for i := Low(Tests) to High(Tests) do begin
    dtu.RegisterTest(SuitePath, Tests[i])
  end
end;
...
begin
  Result := dtu.RunTest(suite, [TTextTestListener.Create]);
...
function RunTests(exitBehavior: TRunnerExitBehavior = rxbContinue): TTestResult;
begin
   Result := RunTest(registeredTests, exitBehavior);
end;
